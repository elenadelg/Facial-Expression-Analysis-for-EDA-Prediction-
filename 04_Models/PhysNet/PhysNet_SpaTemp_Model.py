...
# Input of shape (B, 3, T, H, W) where:
    # B = batch size
    # 3 = RGB channels
    # T = temporal dimension (frames; typically T=1 in this implementation)
    # H, W = height and width of frames

# corresponding kernels applied are of shape  (T, H, W) 
    # t captures the temporal dimension
    # h, w captures the spatial dimension 

# 16 3D convolutional blocks , with 
    # kernel size (varies across layers). 
        # it starts with [1, 5, 5] , then alternates 
        # [1, 3, 3] i.e. focus on  spatial dim
        # [3, 1, 1] i.e. focus on temporal dim.
    # padding (varies across layers - pads the dimension either spatial or temporal according to the kernel focus).
        # it starts with [0, 2, 2], then, following the kernel application, it alternates: 
        # [0, 1, 1] i.e. pads spatial dim
        # [1, 0, 0] i.e. pads temporal dim 
    # relu activation 
    # batch norm
...
class PhysNet_SpaTempBVP(nn.Module):
    ...
    def __init__(self, dropout_rate=0.5):
        ...
        self.ConvBlock1 = nn.Sequential(
            nn.Conv3d(3, 16, [1, 5, 5], stride=1, padding=[0, 2, 2]), # kernel [1, 5, 5] -> focus on spatial dimension (height and width of image), leave temporal dim invariant 
                                                                        # padding [0, 2, 2] -> focus consequentely on spatial dimension 
            nn.BatchNorm3d(16),
            nn.ReLU(inplace=True),
            # nn.LeakyReLU(negative_slope=0.1, inplace=False)
        )
        self.ConvBlock2 = nn.Sequential(
            nn.Conv3d(16, 32, [1, 3, 3], stride=1, padding=[0, 1, 1]), # kernel [1, 3, 3] -> focus on spatial dim, keep temporal  invariant
                                                                        # padding [0, 1, 1] -> as a consequence to the kernel, it focuses on spatial - keep size invariant 
            nn.BatchNorm3d(32),
            nn.ReLU(inplace=True),
            # nn.LeakyReLU(negative_slope=0.1, inplace=False)
        ...
        self.ConvBlock3 = nn.Sequential(
            nn.Conv3d(32, 32, [3, 1, 1], stride=1, padding=[1, 0, 0]), # kernel [3, 1, 1] -> focus on temporal dim, keep spatial invariant
                                                                        # padding [1, 0, 0] -> as a consequence to the kernel, focus on temporal dim
            nn.BatchNorm3d(32),
            nn.ReLU(inplace=True),
            # nn.LeakyReLU(negative_slope=0.1, inplace=False)
        )
        ...
        dummy_output = self.extract_spatiotemporal_features(dummy_input)
            # extract_spatiotemporal_features uses a dummy input during initialization to calculate the flattened feature size dynamically. 
            # This ensures flexibility in processing different input resolutions.
    ...
    def forward(self, sequences, bvp_inputs):
        ...
        flat_features = last_time_step.view(last_time_step.size(0), -1)
                    # flattened spatiotemporal features
        

        # Process BVP => (B, 1) => fc_bvp => (B,32)
        bvp_features = self.fc_bvp(bvp_inputs.unsqueeze(-1))
                    # fully connected layer (fc_bvp) maps the 1D BVP signal to a higher-dimensional space (e.g., (B, 32)), 
