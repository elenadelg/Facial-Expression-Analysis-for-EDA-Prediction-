...
class GradCAM:
    ...
    def generate(self, x):
        ...
        # 1. Perform a forward pass to compute the model's output
        output = self.model(x)  # shape (B, 1)
        # Take the scalar output for a specific class (usually the target class)
        scalar = output[0, 0]  # Assuming B=1

        # 2. Backpropagate to compute gradients of the scalar output w.r.t. feature maps
        scalar.backward(retain_graph=True)

        # 3. Retrieve the saved activations and gradients from the hooks
        activations = self._activations[0]  # shape (num_channels, H', W')
        gradients = self._gradients[0]      # shape (num_channels, H', W')

        # 4. Compute channel-wise importance weights by averaging gradients spatially
        ...
        for c, w in enumerate(weights): # Compute the weighted sum of activations
            grad_cam_map += w * activations[c] # 5. Weighted sum of feature maps
        
        # 6. Apply ReLU to retain only positive contributions
